...
|---|---|---|---|
| 1 | PLT/GOT relocation parsing + import address resolution | `debag: parse PLT/GOT relocations for imports in static-db` | Parses `DT_JMPREL`/`DT_REL`/`DT_RELA` from `.dynamic` and walks each reloc table; `ii` now prints `idx got_addr type name` (e.g. `R_X86_64_JUMP_SLOT  printf`). `s <import_name>` resolves to the GOT slot via the reloc table. PLT calls in `pd`/`pdd` are annotated with `; <name>@plt` (revised from the original `; -> 0xGOT (name)` format by item 2 below) by reading the PLT entry's `jmp [rip+disp32]` and matching the resulting GOT slot to a reloc. ~170 LOC across `static_analysis.c` (reloc parsing) and `static_db.c` (`ii`, `resolve_expr`, PLT annotation). Clean-room reimplementation of `elf_relocs.c:184-240` + `elf_imports.c:400-418`; no rizin code copied. |
| 2 | Jump arrows + call-target annotation in `pd` | `1e7013f` | Two-pass `do_disasm`: (a) collect in-window instruction addresses, (b) for each instruction, classify by mnemonic (`j*` = jump, `call` = call), extract the immediate `0x<target>` if any, and append a trailing `; ...` annotation. Jumps get `; -> <symname>` / `; -> <symname>+0xN` / `; -> 0x<target>`, with ` (out)` appended when the target is outside the disassembly window. Calls get `; <name>@plt` if the target is a PLT entry (resolved via the new `resolve_plt_call` helper, which reads the PLT thunk's `ff 25 <disp32>` and looks up the GOT slot in the reloc table; handles both standard `.plt` and CET `.plt.sec` layouts where `ff 25` is at offset 4 after `endbr64`), or `; <symname>` (or `+0xN`) for direct calls to a known symbol. The previous PLT-call annotation `; -> 0xGOT (name)` format was replaced by `; <name>@plt`. The simpler text-arrow form is used in place of rizin's multi-line ASCII art reflines; full ASCII arrows are a future enhancement. Direct calls to unknown targets and indirect jumps/calls are not annotated. Clean-room reimplementation of `librz/arch/reflines.c:88-285` semantics; no rizin code copied (LGPL-3 vs GPL-2). |
