                ...
                size_t pkg_current, size_t pkg_count)
{
         ...
         * package so the symlink farm can find it later. When the
         * backend is NULL, pacman's default extraction runs as before.
         * See MODIFICATIONS.md #1. */
        if(handle->install_backend != NULL) {
                char *store_path = handle->install_backend(handle, newpkg, pkgfile);
                if(store_path == NULL) {
                        _alpm_log(handle, ALPM_LOG_ERROR,
                                        _("2O9 install backend failed for %s\n"),
                                        newpkg->name);
                ...
                _alpm_log(handle, ALPM_LOG_DEBUG,
                                "2O9: %s installed to %s\n",
                                newpkg->name, store_path);
                /* The store path is owned by the package now. The
                 * backend-allocated string is freed in _alpm_pkg_free()
                 * via the dedicated field we added to alpm_pkg_t (see
                 * MODIFICATIONS.md #1). */
                ...
                /* Emit the same progress events pacman would, so
                 * frontends watching progress don't break. */
