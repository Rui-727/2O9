...
static int local_db_populate(alpm_db_t *db)
{
         ...
         * the 2O9 generation DB. See MODIFICATIONS.md #2. */
        if(db->handle->installed_set_loader != NULL) {
                int rc = db->handle->installed_set_loader(db->handle, db);
                if(rc != 0) {
                        _alpm_log(db->handle, ALPM_LOG_ERROR,
                                  _("2O9: installed_set_loader failed for db '%s'\n"),
                                  db->treename);
                        return -1;
                }
                /* The callback populated the pkgcache directly; nothing
                 * else for us to do. */
                return 0;
        }
