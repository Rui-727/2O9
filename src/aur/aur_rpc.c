                            ...
                            const char *by_field)
{
        ...
        char url[2048];
	char *encoded = curl_easy_escape((CURL *)cache->curl_handle, query, 0);
	if (!encoded) { empty.error = strdup("failed to encode query"); return empty; }
        if (by_field) {
                snprintf(url, sizeof(url), "%s/search/%s?by=%s",
                         cache->base_url, encoded, by_field);
        } else {
                snprintf(url, sizeof(url), "%s/search/%s",
                         cache->base_url, encoded);
        }
	curl_free(encoded);
                                ...
                                const char **names, size_t count)
{
        ...
        /* Build URL: /rpc/v5/info?arg[]=pkg1&arg[]=pkg2... */
        char url[8192] = {0};
        int pos = snprintf(url, sizeof(url), "%s/info", cache->base_url);
